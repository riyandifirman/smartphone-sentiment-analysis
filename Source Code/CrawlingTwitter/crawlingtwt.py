...
dictTweets = {'waktu': tweet_loc, 'id': tweet_id,
              'username': tweet_user, 'tweet': tweet_list}
...
if keyword != '':
    if naOfTweet != 0:
        ...
        df_fix['text_preprocessed'] = df_fix['text_preprocessed'].apply(
            filteringText)
        ...
        with col2:
            ...
            st.write(df_fix_positive[['text', 'sentimen_score']].sort_values(
                'sentimen_score', ascending=False))
        with col3:
            st.header("Negatif")
            st.image("wc_negatif.png")
            st.write(df_fix_negative[['text', 'sentimen_score']].sort_values(
                'sentimen_score', ascending=True))

        # Function to Create Csv
        def create_csv():
            df_fix.to_csv('hasil_sentimen.csv', index=False)
            st.write("CSV Created")
        create_csv()
